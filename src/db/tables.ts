...
export const teamAgentsCacheTable = sqliteTable('team_agents_cache', {
  ...
  mcpKinds: text('mcp_kinds', { mode: 'json' }).$type<string[]>(),
  toolKinds: text('tool_kinds', { mode: 'json' }).$type<string[]>(),
  accepts: text('accepts', { mode: 'json' }).$type<string[]>(),
  modes: text('modes', { mode: 'json' }).$type<string[]>(),
  agentSkillCount: integer('agent_skill_count'),
